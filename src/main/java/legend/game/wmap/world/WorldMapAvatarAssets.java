 ...
 */
public record WorldMapAvatarAssets(CContainer model, @Nullable Tim texture, List<TmdAnimationFile> animations, WorldMapPoint scale, float shadowScale, int idleAnimation, int walkAnimation, int runAnimation, int textureSlot) {
  public WorldMapAvatarAssets {
    Objects.requireNonNull(model, "model");
    animations = List.copyOf(animations);
    Objects.requireNonNull(scale, "scale");
    if(textureSlot < 0 || textureSlot > 3) throw new IllegalArgumentException("World map avatar texture slot must be 0..3");
    if(textureSlot == 0 && texture == null || textureSlot != 0 && texture != null) throw new IllegalArgumentException("Only leader-slot avatars provide an uploadable TIM");
    ...
    for(final int animation : new int[]{idleAnimation, walkAnimation, runAnimation}) {
      if(animation < 0 || animation >= animations.size()) throw new IllegalArgumentException("World map avatar animation index outside loaded animations: " + animation);
    }
  }
