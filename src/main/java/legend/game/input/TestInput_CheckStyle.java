...
public final class TestInput_CheckStyle {
  ...
  public static void update() {
    ...
    if(Input.releasedThisFrame(InputAction.BUTTON_NORTH)) {
      LOGGER.info("NORTH BUTTON RE-LEASED THIS FRAME");
    }
    if(Input.releasedThisFrame(InputAction.BUTTON_SOUTH)) {
      LOGGER.info("SOUTH BUTTON RE-LEASED THIS FRAME");
    }
    if(Input.releasedThisFrame(InputAction.BUTTON_EAST)) {
      LOGGER.info("EAST BUTTON RE-LEASED THIS FRAME");
    }
    if(Input.releasedThisFrame(InputAction.BUTTON_WEST)) {
    ...
    if(Input.pressedThisFrame(InputAction.BUTTON_THUMB_1)) {
      LOGGER.info("THUMB STICK 1 PRESSED THIS FRAME");
    }

    if(Input.pressedThisFrame(InputAction.BUTTON_THUMB_2)) {
  ...
  private static void logDpad() {
    if(Input.pressedThisFrame(InputAction.DPAD_UP)) {
      LOGGER.info("DPAD UP PRESSED THIS FRAME");
    }
    if(Input.pressedThisFrame(InputAction.DPAD_DOWN)) {
      LOGGER.info("DPAD DOWN PRESSED THIS FRAME");
    }
    if(Input.pressedThisFrame(InputAction.DPAD_LEFT)) {
      LOGGER.info("DPAD LEFT PRESSED THIS FRAME");
    }
    if(Input.pressedThisFrame(InputAction.DPAD_RIGHT)) {
      LOGGER.info("DPAD RIGHT PRESSED THIS FRAME");
    }

    if(Input.getButtonState(InputAction.DPAD_LEFT) && Input.getButtonState(InputAction.DPAD_UP)) {
      LOGGER.info("DPAD LEFT UP PRESSED (DIAGONAL)");
    }
  }

  private static void logLeftStickAxisAsButton() {
    if(Input.getButtonState(InputAction.JOYSTICK_LEFT_BUTTON_UP)) {
      LOGGER.info("UP ON JOYSTICK LEFT TREATED AS BUTTON");
    }
    if(Input.getButtonState(InputAction.JOYSTICK_LEFT_BUTTON_DOWN)) {
      LOGGER.info("DOWN ON JOYSTICK LEFT TREATED AS BUTTON");
    }
    if(Input.getButtonState(InputAction.JOYSTICK_LEFT_BUTTON_LEFT)) {
      LOGGER.info("LEFT ON JOYSTICK LEFT TREATED AS BUTTON");
    }
    if(Input.getButtonState(InputAction.JOYSTICK_LEFT_BUTTON_RIGHT)) {
      LOGGER.info("RIGHT ON JOYSTICK LEFT TREATED AS BUTTON");
    }
  }

  private static void logRightStickAxisAsButton() {
    if(Input.getButtonState(InputAction.JOYSTICK_RIGHT_BUTTON_UP)) {
      LOGGER.info("UP ON JOYSTICK RIGHT TREATED AS BUTTON");
    }
    if(Input.getButtonState(InputAction.JOYSTICK_RIGHT_BUTTON_DOWN)) {
      LOGGER.info("DOWN ON JOYSTICK RIGHT TREATED AS BUTTON");
    }
    if(Input.getButtonState(InputAction.JOYSTICK_RIGHT_BUTTON_LEFT)) {
      LOGGER.info("LEFT ON JOYSTICK RIGHT TREATED AS BUTTON");
    }
    if(Input.getButtonState(InputAction.JOYSTICK_RIGHT_BUTTON_RIGHT)) {
      LOGGER.info("RIGHT ON JOYSTICK RIGHT TREATED AS BUTTON");
    }
  }

  private static void logLeftStickAxisAsButton_PressedThisFrame() {
    if(Input.pressedThisFrame(InputAction.JOYSTICK_LEFT_BUTTON_UP)) {
      LOGGER.info("UP ON JOYSTICK LEFT TREATED AS BUTTON - PRESSED THIS FRAME");
    }
    if(Input.pressedThisFrame(InputAction.JOYSTICK_LEFT_BUTTON_DOWN)) {
      LOGGER.info("DOWN ON JOYSTICK LEFT TREATED AS BUTTON - PRESSED THIS FRAME");
    }
    if(Input.pressedThisFrame(InputAction.JOYSTICK_LEFT_BUTTON_LEFT)) {
      LOGGER.info("LEFT ON JOYSTICK LEFT TREATED AS BUTTON - PRESSED THIS FRAME");
    }
    if(Input.pressedThisFrame(InputAction.JOYSTICK_LEFT_BUTTON_RIGHT)) {
  ...
  private static void logLeftStickAxisState() {
    if(Input.getAxisState(InputAction.JOYSTICK_LEFT_X) == InputAxisState.AXIS_POSITIVE) {
      LOGGER.info("LEFT STICK X POSITIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_LEFT_X) == InputAxisState.AXIS_NEGATIVE) {
      LOGGER.info("LEFT STICK X NEGATIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_LEFT_X) == InputAxisState.AXIS_CENTERED) {
      //LOGGER.info("LEFT STICK X CENTERED, will spam logger");
    }

    if(Input.getAxisState(InputAction.JOYSTICK_LEFT_Y) == InputAxisState.AXIS_POSITIVE) {
      LOGGER.info("LEFT STICK Y POSITIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_LEFT_Y) == InputAxisState.AXIS_NEGATIVE) {
      LOGGER.info("LEFT STICK Y NEGATIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_LEFT_Y) == InputAxisState.AXIS_CENTERED) {
  ...
  private static void logRightStickAxisState() {
    if(Input.getAxisState(InputAction.JOYSTICK_RIGHT_X) == InputAxisState.AXIS_POSITIVE) {
      LOGGER.info("RIGHT STICK X POSITIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_RIGHT_X) == InputAxisState.AXIS_NEGATIVE) {
      LOGGER.info("RIGHT STICK X NEGATIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_RIGHT_X) == InputAxisState.AXIS_CENTERED) {
      //LOGGER.info("RIGHT STICK X CENTERED, will spam logger");
    }

    if(Input.getAxisState(InputAction.JOYSTICK_RIGHT_Y) == InputAxisState.AXIS_POSITIVE) {
      LOGGER.info("RIGHT STICK Y  POSITIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_RIGHT_Y) == InputAxisState.AXIS_NEGATIVE) {
      LOGGER.info("RIGHT STICK Y NEGATIVE");
    }
    if(Input.getAxisState(InputAction.JOYSTICK_RIGHT_Y) == InputAxisState.AXIS_CENTERED) {
  ...
  private static void logRightStickAxisAsButton_PressedThisFrame() {
    ...
    if(Input.pressedThisFrame(InputAction.JOYSTICK_RIGHT_BUTTON_LEFT)) {
