...
public final class V4Serializer {
  ...
  public static SavedGame fromV4(final Campaign campaign, final String filename, final FileData data) {
    final IntRef offset = new IntRef();
    final String name = data.readAscii(offset);

    final ConfigCollection config = new ConfigCollection();
    final RetailSavedGame savedGame = new RetailSavedGame(campaign, "V4", filename, name, LodMod.RETAIL_CAMPAIGN_TYPE.getId(), config);
    ...
    for(int i = 0; i < itemCount; i++) {
      ...
      savedGame.itemIds.add(new InventoryEntry(itemId, 1, 1));
    }
