...
public final class V5Serializer {
  ...
  public static SavedGame<SaveDisplay> fromV5(final String filename, final FileData data) {
    ...
    final SaveType saveType = REGISTRIES.saveTypes.getEntry(saveTypeId).get();
    final SaveDisplay display = saveType.deserialize(data.slice(offset.get()), offset);
    ...
    return new SavedGame<>(filename, name, saveType, display, state, config);
  }
