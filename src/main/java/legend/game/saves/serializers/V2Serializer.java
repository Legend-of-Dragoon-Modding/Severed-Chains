...
public final class V2Serializer {
  ...
  public static SavedGame fromV2(final Campaign campaign, final String name, final FileData data) {
    final ConfigCollection config = new ConfigCollection();
    final RetailSavedGame savedGame = new RetailSavedGame(campaign, "V2", name, name, LodMod.RETAIL_CAMPAIGN_TYPE.getId(), config);
    ...
    for(int i = 0; i < 2; i++) {
      ...
      for(int bit = 0; bit < 32; bit++) {
        if((packed & (1 << bit)) != 0 && i * 32 + bit < LodMod.GOODS_IDS.length) {
    ...
    for(int charIndex = 0; charIndex < savedGame.charStats.length; charIndex++) {
      ...
      for(int i = 0; i < 5; i++) {
        charData.equipmentIds.put(EquipmentSlot.fromLegacy(i), LodMod.id(LodMod.EQUIPMENT_IDS[data.readUByte(offset)]));
      }
