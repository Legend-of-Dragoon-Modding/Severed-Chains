...
public class RecoverHpItem extends BattleItem {
  ...
  public boolean canBeUsedNow(final ItemStack stack, final UsageLocation location) {
    for(int i = 0; i < characterIndices_800bdbb8.size(); i++) {
      if(!gameState_800babc8.charData_32c.get(characterIndices_800bdbb8.getInt(i)).stats.getStat(HP_STAT.get()).isFull()) {
        return true;
      }
    }
  ...
  public void useInMenu(final ItemStack stack, final UseItemResponse response, final int charId) {
    ...
    if(this.percentage == 100) {
      ...
      amount = gameState_800babc8.charData_32c.get(charId).stats.getStat(HP_STAT.get()).getMax() * this.percentage / 100;
    }
  ...
  public int calculateStatMod(final ItemStack stack, final BattleEntity27c user, final BattleEntity27c target) {
    ...
    return target.stats.getStat(HP_STAT.get()).getMax() * this.percentage / 100;
  }
