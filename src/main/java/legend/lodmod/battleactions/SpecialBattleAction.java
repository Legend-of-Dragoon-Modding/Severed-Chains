...
public class SpecialBattleAction extends BattleAction {
  ...
  public void draw(final Battle battle, final int index, final boolean selected) {
    ...
    if(selected && menu.renderSelectedIconText_40) {
      ...
      this.index = 0;
      renderText(I18n.translate(this), menuElementBaseX + ICON_SIZE / 2.0f, menu.y_08 - 24.0f, FONT, (model, shadow) -> {
        if(!shadow) {
          MathHelper.hsvToRgb((tickCount_800bb0fc + this.index * 10) % 100 / 100.0f, 0.5f, 1.0f, this.colour);
