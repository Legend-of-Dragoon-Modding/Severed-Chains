...
public final class Breath {
  ...
  private Breath(final int waveformIndex, final boolean lerpInterp, final boolean half, final boolean abs) {
  ...
  public float getValue(final int position, final int interpolationIndex, final LookupTables lookupTables) {
    ...
    float breath;
    if(this.lerpInterp) {
      breath = waveforms[this.waveformIndex][position] + (float)interpolationIndex / lookupTables.getInterpolationStep() * (waveforms[this.waveformIndex][position + 1] - waveforms[this.waveformIndex][position]);
    } else {
      breath = lookupTables.interpolate(waveforms[this.waveformIndex], position, interpolationIndex);
    }
  ...
  public static long convert(final int value) {
    return Math.round(BREATH_MAX_VALUE / (60 - 58 * value / 127.0d));
  }
  ...
  private static void addBreath(final int[] data, final int waveformIndex, final boolean lerpInterp, final boolean half, final boolean abs) {
    final int hash = hash(data);

    map.put(hash, new Breath(waveformIndex, lerpInterp, half, abs));
  }
  ...
  public static void initializeWaveforms(final Interpolation interpolation) {
    // Sine waves
    waveforms[0] = new float[BREATH_COUNT + interpolation.taps - 1];
    waveforms[2] = new float[BREATH_COUNT + interpolation.taps - 1];

    final int offset = interpolation.taps / 2 - 1;
    for(int i = 1; i < BREATH_COUNT; i++) {
      waveforms[0][i + offset] = sineWave(i, BREATH_COUNT);
      waveforms[2][i + offset] = sineWave(i * 2, BREATH_COUNT);
    }

    // Copy back to front and front to back for interpolation
    for(int i = 0; i < offset; i++) {
      waveforms[0][i] = waveforms[0][BREATH_COUNT + offset + i];
      waveforms[2][i] = waveforms[2][BREATH_COUNT + offset + i];
      waveforms[0][BREATH_COUNT + offset + i + 1] = waveforms[0][offset + i + 1];
      waveforms[2][BREATH_COUNT + offset + i + 1] = waveforms[2][offset + i + 1];
    }

    // Triangular waves are lerped, so they don't care about the interpolation setting
