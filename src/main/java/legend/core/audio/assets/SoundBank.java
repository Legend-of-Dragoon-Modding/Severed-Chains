...
final class SoundBank {
  ...
  SoundBank(final FileData data) {
    ...
    for(int entry = offsets.size() - 1; entry >= 0; entry--) {
      ...
      this.entries.put(offset, generateEntry(data.slice(offset, size)));
      lastOffset = offset;
    }
  }

  Pair<short[][], int[]> getEntry(final int offset) {
  ...
  private static Pair<short[][], int[]> generateEntry(final FileData data) {
    final int blockCount = data.size() / 16;
    ...
    for(int block = 0; block < blockCount; block++) {
      flag[block] = data.readUByte(block * 16 + 1);

      final int shift = 12 - (data.readUByte(block * 16) & 0xf);
      int filter = (data.readUByte(block * 16) & 0x70) >> 4; //filter on SPU adpcm is 0-4 vs XA which is 0-3
      if(filter > 4) {
      ...
      for(int i = 0; i < 28; i++) {
        ...
        final int t = signed4bit((byte)(data.readUByte(block * 16 + position) >> nibble * 4 & 0x0f));
