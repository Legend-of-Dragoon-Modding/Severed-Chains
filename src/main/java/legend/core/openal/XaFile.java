...
import static org.lwjgl.system.MemoryStack.stackMallocShort;
import static org.lwjgl.system.MemoryStack.stackPop;
import static org.lwjgl.system.MemoryStack.stackPush;

public class XaFile {
  final static int OGG_SAMPLE_LIMIT = 32;
  ...
  public XaFile(final String filePath, final boolean loops) {
    ...
    final int limit = (int) Math.ceil((double) sampleCount / OGG_SAMPLE_LIMIT);

    stackPush();
    final ShortBuffer pcmBuffer = stackMallocShort(OGG_SAMPLE_LIMIT);
    stackPush();
    final IntBuffer li = stackMallocInt(1);

    for(int i = 0; i < limit; i++) {
      pcmBuffer.position(0);
      OpusFile.op_read(oggFile, pcmBuffer, li);
      pcmBuffer.position(0);
      pcmBuffer.get(0, rawPcm, i * OGG_SAMPLE_LIMIT, Math.min(OGG_SAMPLE_LIMIT, sampleCount - i * OGG_SAMPLE_LIMIT));
    }

    System.out.println(Arrays.toString(rawPcm));
