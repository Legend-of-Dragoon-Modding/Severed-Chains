...
flat in highp vec2 vertTpage;
flat in highp vec2 vertClut;
flat in int vertBpp;
smooth in highp vec4 vertColour;
flat in int vertFlags;

flat in highp float widthMultiplier;
flat in int widthMask;
flat in int indexShift;
flat in int indexMask;

smooth in highp float depth;
smooth in highp float depthOffset;
...
void main() {
  ...
  bool translucent = (vertFlags & 0x8) != 0 || translucency != 0.0;
  ...
  if(textured) {
    ...
    if(texColour.a == 0.0 && (useTextureAlpha != 0.0 || texColour.r == 0.0 && texColour.g == 0.0 && texColour.b == 0.0)) {
      discard;
    }
